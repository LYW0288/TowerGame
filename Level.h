#ifndef LEVEL_H_INCLUDED
#define LEVEL_H_INCLUDED

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <string>
#include <vector>
#include "global.h"

typedef struct Node {
    int h, g, f;
    bool roadPoint;
    bool treePoint;
    bool towerPoint;
}Node;

class LEVEL {
public:
    LEVEL(const int);
    ~LEVEL();

    void setLevel(const int);
    void settower(int i) { levelMap[i].towerPoint = true; }
    void setRecord(int, int, int, int);
    bool isRoad(int index) { return levelMap[index].roadPoint; }
    bool isTree(int index) { return levelMap[index].treePoint; }
    bool isTower(int index) { return levelMap[index].towerPoint; }
    bool isInit(int index) { return init[index]; }
    int getMonsterSpeed() { return MonsterSpeed; }
    int getLevel() { return level; }
    //void search_destination();
    std::vector<int> ReturnPath() { return road_grid; }

    int MonsterNum[300];
    int Monster_MAX;
    int heart[4], coin[4], score[4];
    bool init[4] = {false};

private:
    Node levelMap[NumOfGrid];
    std::vector<int> road_grid;
    // current level number
    int level = 1;
    // maximum number of monster of a level
    // frequency of producing  monster
    int MonsterSpeed = 80;
};


#endif // LEVEL_H_INCLUDED
