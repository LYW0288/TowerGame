...
Menu::Menu()
{
    ...
    menuFont = al_load_ttf_font("./Title/misaki_gothic.ttf", 24, 0); // load font
}
...
Menu::Draw()
{
    ...
    sprintf(buffer, "x%d", HealthPoint);
    al_draw_bitmap(love, offsetX, 20, 0);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), 2*al_get_bitmap_width(love) + offsetX-8, 20, 0, buffer);
    ...
    for(int i=0; i < Num_TowerType; i++)
    {
        ...
        if(!Enough_Coin(i))
            ...
            al_draw_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgb(255, 0, 0), 4);
            al_draw_textf(menuFont, al_map_rgb(255, 255, 255), offsetX, 20 + 15*gapY, ALLEGRO_ALIGN_LEFT, "Coin:%d", need_coin[i]);
            al_draw_textf(menuFont, al_map_rgb(255, 255, 255), offsetX, 20 + 16*gapY, ALLEGRO_ALIGN_LEFT, "HP  :%d", HP[i]);
            al_draw_textf(menuFont, al_map_rgb(255, 255, 255), offsetX, 20 + 17*gapY, ALLEGRO_ALIGN_LEFT, "ATK :%d", ATK[i]);
        }
        else
            al_draw_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgb(255, 255, 255), 4);
    }
}
